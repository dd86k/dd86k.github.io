/*
CKSUM algorithm implementation in Javascript.
Copyrights DD~! (c) 2015
*/
"use strict";
...
function HashTextCKSUM(pText)
{
    ...
    return tmp + " " + pText.length + " " + pText;
}

function UpdateCKSUM(s)
{
    var len = s.length || 0;

    for (var i = 0; i < len; i++)
    {
        tmp = (tmp << 8) ^ crctabcksum[((tmp >> 24) ^ s[i].charCodeAt()) & 0xFF];
    }

    for (; len != 0; len >>= 8)
    {
        tmp = (tmp << 8) ^ crctabcksum[((tmp >> 24) ^ (len & 0xFF)) & 0xFF];
    }
}